...
exports.getAllNotifications = async (req, res) => {
 try {
    ...
    const notifications = await Notification.find().populate('userId', 'firstName lastName userImage email').sort({ createdAt: -1 });
    if (!notifications) {
...
exports.markNotificationAsRead = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Notification marked as read successfully!", notification });
  } catch (error) {
