...
exports.registerForCourse = async (req, res) => {
  try {
    ...
    if (!registration) {
      return res
        .status(400)
        .json({ success: false, message: "Course registration failed!" });
    }
...
exports.getRegisteredCourses = async (req, res) => {
  try {
    ...
    if (!registrations || registrations.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No registered course found!" });
    }
    const registrationIds = registrations.map((reg) => reg.course._id);
    const courses = await Course.find({ _id: { $in: registrationIds } }).sort(
      "createdAt:-1"
    );
    ...
    if (!courses || courses.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No registered courses found!" });
    }

    res.status(200).json({
      success: true,
      message: "Successfully fetched all registered courses",
      courses,
    });
  } catch (error) {
...
exports.getRegisteredUsers = async (req, res) => {
  try {
    ...
    const registrations = await CourseRegistration.find({
      course: courseId,
    }).populate("enrolledUser");
    if (!registrations || registrations.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No users found for this course!" });
    }
    const registrationIds = registrations.map((reg) => reg.enrolledUser);

    const users = await User.find({ _id: { $in: registrationIds } }).sort(
      "createdAt:-1"
    );
...
exports.createCourse = async (req, res) => {
  try {
    ...
    if (!title || !duration) {
      return res.status(400).json({
        success: false,
        message: "course title and duration are required!",
      });
    }
...
exports.getOtherCourses = async (req, res) => {
  try {
    ...
    const registrationIds = registrations.map((reg) => reg.course._id);
    const courses = await Course.find({ _id: { $nin: registrationIds } }).sort(
      "createdAt: -1"
    );
    ...
    if (!courses || courses.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No other courses found!" });
    }
    res.status(200).json({
      success: true,
      message: "Successfully fetched other courses",
      courses,
    });
  } catch (error) {
...
exports.registerForOtherCourses = async (req, res) => {
  try {
    ...
    res.status(201).json({
      success: true,
      message: "This course is successfully registered",
      registration: otherCourse,
      user,
    });
  } catch (error) {
...
exports.getDashboardMetrics = async (req, res) => {
  try {
    const [totalCourses, totalUsers, totalRegistrations, totalWorkshops] =
      await Promise.all([
    ...
    res.status(200).json({
      success: true,
      message: "Successfully fetched dashboard metics",
      metrics: {
        totalCourses,
        totalUsers,
        totalRegistrations,
...
exports.getRegisteredUsersByAdmin = async (req, res) => {
  try {
    ...
    }).populate("registeredUsers", "firstName lastName email userImage");
    if (!courses || courses.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No registered users found!" });
    ...
    const totalStudents = courses.reduce(
      (acc, course) => acc + course.registeredUsers.length,
      0
    );
